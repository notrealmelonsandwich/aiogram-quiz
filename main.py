...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await Form.next()
    await message.reply("Какое у тебя имя?")
...
@dp.message_handler(lambda message: message.text not in ["2017", "2018", "2019", "2020"], state=Form.group)
...
async def process_age(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await bot.send_message(
            message.chat.id,
            md.text(
                md.text('Рад познакомиться,', md.bold(data['first_name'] + " " + data['last_name'])),
